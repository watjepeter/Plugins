...
class SplineSelMod : public Modifier, public ISplineSelect {   
   public:           
      DWORD selLevel;
      Tab<TSTR*> namedSel[3];    
      ...
      const TCHAR *GetObjectName() {return GetString(IDS_AP_SPLINESELMOD);}
      void *GetInterface(ULONG id) { if (id==I_SPLINESELECT) return (ISplineSelect*)this; else return Modifier::GetInterface(id); }

      // From modifier
      ChannelMask ChannelsUsed()  {return GEOM_CHANNEL|TOPO_CHANNEL|SELECT_CHANNEL|SUBSEL_TYPE_CHANNEL;}
      ChannelMask ChannelsChanged() {return SELECT_CHANNEL;}
      ...
      // CAL-03/11/03: implement ISplineSelect interface
      // From ISplineSelect
      DWORD GetSelLevel();
...
class SplineSelData : public LocalModData, public ISplineSelectData {
   public:
      // LocalModData
      void* GetInterface(ULONG id) { if (id == I_SPLINESELECTDATA) return (ISplineSelectData*)this; else return LocalModData::GetInterface(id); }
      ...
      // CAL-03/11/03: implement ISplineSelectData interface
      GenericNamedSelSetList vselSet;        // cache of vertSel
      GenericNamedSelSetList sselSet;        // cache of SegSel
      GenericNamedSelSetList pselSet;        // cache of SplineSel

      // From ISplineSelectData
      BitArray GetVertSel();
      BitArray GetSegmentSel();
      BitArray GetSplineSel();
      
      void SetVertSel(BitArray &set, ISplineSelect *imod, TimeValue t);
      void SetSegmentSel(BitArray &set, ISplineSelect *imod, TimeValue t);
      void SetSplineSel(BitArray &set, ISplineSelect *imod, TimeValue t);

      GenericNamedSelSetList & GetNamedVertSelList ();
      GenericNamedSelSetList & GetNamedSegmentSelList ();
      GenericNamedSelSetList & GetNamedSplineSelList ();
   };
...
static INT_PTR CALLBACK SplineSelObjectProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
static INT_PTR CALLBACK SplineSelVertProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
static INT_PTR CALLBACK SplineSelSegProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
static INT_PTR CALLBACK SplineSelProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
void SplineSelMod::NotifyInputChanged(const Interval& changeInt, PartID partID, RefMessage message, ModContext *mc)
   {
   if (mc->localData && ip) {
...
void SplineSelMod::UpdateNamedSelDropDown () {
   if (!ip) return;
   ...
   BitArray nselmatch;
   nselmatch.SetSize (namedSel[selLevel-1].Count());
   nselmatch.SetAll ();
   int nd, i, foundone=FALSE;
   for (nd=0; nd<mcList.Count(); nd++) {
      SplineSelData *d = (SplineSelData *) mcList[nd]->localData;
      if (!d) continue;
      foundone = TRUE;
      switch (selLevel) {
      case SEL_VERTEX:
         for (i=0; i<nselmatch.GetSize(); i++) {
            ...
            if (!(*(d->NvselSet.sets[i]) == d->vertSel)) nselmatch.Clear(i);
         }
         break;
      case SEL_SEGMENT:
         for (i=0; i<nselmatch.GetSize(); i++) {
            if (!nselmatch[i]) continue;
            if (!(*(d->NsegselSet.sets[i]) == d->SegSel)) nselmatch.Clear(i);
         }
         break;
      case SEL_SPLINE:
         for (i=0; i<nselmatch.GetSize(); i++) {
            if (!nselmatch[i]) continue;
            if (!(*(d->NsplselSet.sets[i]) == d->SplineSel)) nselmatch.Clear(i);
         }
         break;
      }
   ...
   if (foundone && nselmatch.NumberSet ()) {
      ...
      ip->SetCurNamedSelSet (*(namedSel[selLevel-1][i]));
   } else ip->ClearCurNamedSelSet ();
}

DWORD SplineSelMod::GetSelLevel () {
   switch (selLevel) {
   case SEL_VERTEX:  return SS_VERTEX;
   case SEL_SEGMENT: return SS_SEGMENT;
   case SEL_SPLINE:  return SS_SPLINE;
   default:       return SS_OBJECT;
   }
}

void SplineSelMod::SetSelLevel(DWORD sl) {
   switch (sl) {
   case SS_OBJECT:
      selLevel = SEL_OBJECT;
      break;
...
void SplineSelMod::LocalDataChanged() {
   NotifyDependents(FOREVER, PART_SELECT, REFMSG_CHANGE);
   if (ip && editMod==this) {
      UpdateNamedSelDropDown ();
   }
...
static INT_PTR CALLBACK SplineSelObjectProc(
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
   {
   SplineSelMod *mod = DLGetWindowLongPtr<SplineSelMod*>(hWnd);
   switch (msg) {
      case WM_INITDIALOG:
...
static INT_PTR CALLBACK SplineSelVertProc(
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
   {
   SplineSelMod *mod = DLGetWindowLongPtr<SplineSelMod*>(hWnd);
   switch (msg) {
...
static INT_PTR CALLBACK SplineSelSegProc(
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
   {
   SplineSelMod *mod = DLGetWindowLongPtr<SplineSelMod*>(hWnd);
   switch (msg) {
...
static INT_PTR CALLBACK SplineSelProc(
      HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
   {
   SplineSelMod *mod = DLGetWindowLongPtr<SplineSelMod*>(hWnd);
   switch (msg) {
...
// CAL-03/11/03: implement ISplineSelectData interface
BitArray SplineSelData::GetVertSel()
{
   BitArray sel;
   int poly, size, n;
   
   for (poly = 0, size = 0; poly < vertSel.polys; poly++)
      size += vertSel[poly].GetSize();
   sel.SetSize(size);

   for (poly = 0, n = 0; poly < vertSel.polys; poly++) {
      for (int v = 0; v < vertSel[poly].GetSize(); v++, n++)
         sel.Set(n, vertSel[poly][v]);
   }
   return sel;
}

BitArray SplineSelData::GetSegmentSel()
{
   BitArray sel;
   int poly, size, n;
   
   for (poly = 0, size = 0; poly < SegSel.polys; poly++)
      size += SegSel[poly].GetSize();
   sel.SetSize(size);

   for (poly = 0, n = 0; poly < SegSel.polys; poly++) {
      for (int v = 0; v < SegSel[poly].GetSize(); v++, n++)
         sel.Set(n, SegSel[poly][v]);
   }
   return sel;
}

BitArray SplineSelData::GetSplineSel()
{
   return SplineSel.sel;
}

void SplineSelData::SetVertSel(BitArray &set, ISplineSelect *imod, TimeValue t)
{
   SplineSelMod *mod = (SplineSelMod *) imod;
   int size = set.GetSize(), poly, n;
   
   if (theHold.Holding()) theHold.Put (new SplSelRestore (mod, this));
   for (poly = 0, n = 0; poly < vertSel.polys; poly++)
      for (int v = 0; v < vertSel[poly].GetSize(); v++, n++)
         vertSel[poly].Set(v, (n<size) ? set[n] : 0);
   if (shape) shape->vertSel = vertSel;
}

void SplineSelData::SetSegmentSel(BitArray &set, ISplineSelect *imod, TimeValue t)
{
   SplineSelMod *mod = (SplineSelMod *) imod;
   int size = set.GetSize(), poly, n;
   ...
   for (poly = 0, n = 0; poly < SegSel.polys; poly++)
      for (int v = 0; v < SegSel[poly].GetSize(); v++, n++)
         SegSel[poly].Set(v, (n<size) ? set[n] : 0);
   if (shape) shape->segSel = SegSel;
}

void SplineSelData::SetSplineSel(BitArray &set, ISplineSelect *imod, TimeValue t)
{
   SplineSelMod *mod = (SplineSelMod *) imod;
   if (theHold.Holding()) theHold.Put (new SplSelRestore (mod, this));
   SplineSel.sel = set;
   if (shape) shape->polySel = SplineSel;
}

GenericNamedSelSetList & SplineSelData::GetNamedVertSelList ()
{
   for (int i = vselSet.Count() - 1; i >= 0; i--)
      vselSet.DeleteSet(i);

   for (int i = 0; i < NvselSet.Count(); i++) {
      BitArray sel;
      ShapeVSel &shpsel = NvselSet[i];
      int poly, size, n;
      
      for (poly = 0, size = 0; poly < shpsel.polys; poly++)
         size += shpsel[poly].GetSize();
      sel.SetSize(size);

      for (poly = 0, n = 0; poly < shpsel.polys; poly++) {
         for (int v = 0; v < shpsel[poly].GetSize(); v++, n++)
            sel.Set(n, shpsel[poly][v]);
      }

      vselSet.AppendSet(sel);
   }

   return vselSet;
}

GenericNamedSelSetList & SplineSelData::GetNamedSegmentSelList ()
{
   for (int i = sselSet.Count() - 1; i >= 0; i--)
      sselSet.DeleteSet(i);

   for (int i = 0; i < NsegselSet.Count(); i++) {
      BitArray sel;
      ShapeSSel &shpsel = NsegselSet[i];
      int poly, size, n;
      ...
      sselSet.AppendSet(sel);
   }

   return sselSet;
}

GenericNamedSelSetList & SplineSelData::GetNamedSplineSelList ()
{
   for (int i = pselSet.Count() - 1; i >= 0; i--)
      pselSet.DeleteSet(i);

   for (int i = 0; i < NsplselSet.Count(); i++)
      pselSet.AppendSet(NsplselSet[i].sel);
...
int SplineSelMod::NumSubObjTypes() 
{ 
   return 3;
}

ISubObjType *SplineSelMod::GetSubObjType(int i) 
{
   ...
   {
      initialized = true;
      SOT_Vertex.SetName(GetString(IDS_AP_VERTEX));
      SOT_Segment.SetName(GetString(IDS_AP_SEGMENT));
      SOT_Spline.SetName(GetString(IDS_AP_SPLINE));
   }
